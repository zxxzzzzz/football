...
async function getGameListByNodeFetch(url: string, ver: string, uid: string, lid: string): Promise<Game[]> {
  ...
  try {
    ...
    // @ts-ignore
    throw createError('获取extra 比赛数据失败 网络问题' + _url.origin + error.message, Code.dataFail);
  }
  if (!text) {
...
async function getServiceMainget(ver: string) {
  ...
  try {
    const res = await axios.post(`https://61.14.172.140/transform.php?ver=${ver}`, `p=service_mainget&ver=${ver}&langx=zh-cn&login=N`, {
      headers: {
        accept: '*/*',
        'accept-language': 'zh-CN,zh;q=0.9',
        'cache-control': 'no-cache',
