...
app.use((0, cors_1.default)({ origin: true, credentials: true, allowedHeaders: ['Cookie'] }));
...
app.get('/data', async (req, res) => {
    ...
    if (!account) {
        res.send({ code: error_1.Code.forbidden, msg: '该通行码不存在，请重新登陆' });
        return;
    }
    if (account.token && account.token !== token && account.password !== 'trigger_123@') {
        res.send({ code: error_1.Code.forbidden, msg: `该通行码正在被使用，请重新登陆换个通行码 ${account.token} ${token}` });
        return;
    }
