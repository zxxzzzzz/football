...
export async function getGameList(page: Page, ver: string, uid: string, lid: string): Promise<Game[]> {
  const body = {
    uid: uid,
    ver: ver,
    langx: 'zh-cn',
  ...
  const text = await page.evaluate(async (bodyStr) => {
    const res = await fetch(`${location.origin}/transform.php?ver=${ver}`, {
      headers: {
        accept: '*/*',
...
export const retryGetGameList = retryWrap(getGameList, 3);
export async function getGameOBT(
  page: Page,
  ver: string,
  uid: string,
  ecid: string
...
export const retryGetGameOBT = retryWrap(getGameOBT, 3);

export async function getLeagueListAll(page: Page, uid: string, ver: string): Promise<{ name: string; id: string }[]> {
  const body = {
...
export const retryGetLeagueListAll = retryWrap(getLeagueListAll, 3);
